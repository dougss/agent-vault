...
TASK_COUNT=$(grep -c '^- \[ \] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
...
### Step 2: Verify
Run: \`$RUNNER tsc --noEmit && $RUNNER vitest run && $RUNNER eslint . --quiet\`
...
while [[ $ATTEMPT -lt $MAX_RETRIES ]]; do
  ...
  PENDING_BEFORE=$(grep -c '^\- \[ \] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
  ...
  if [[ $ATTEMPT -gt 1 ]]; then
    DONE_SO_FAR=$(grep -c '^\- \[x\] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
...
Focus on the NEXT unchecked task in tasks.md."
  ...
  DONE_AFTER=$(grep -c '^\- \[x\] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
  PENDING_AFTER=$(grep -c '^\- \[ \] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
  PROGRESS_THIS_ATTEMPT=$((DONE_AFTER - ${DONE_SO_FAR:-0}))

  # Check for new files even if tasks.md wasn't updated
  NEW_FILES=$(git -C "$PROJECT_DIR" ls-files --others --exclude-standard 2>/dev/null | grep -cE '\.(ts|tsx|js|jsx|css|sql)$' || echo 0)
...
# --- Final counts ---
DONE_COUNT=$(grep -c '^\- \[x\] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
PENDING_COUNT=$(grep -c '^\- \[ \] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
...
COMMITS_AHEAD=$(git -C "$PROJECT_DIR" rev-list --count "origin/main..HEAD" 2>/dev/null || echo 0)
...
# --- Create PR ---
ISSUE_REF=$(grep -oE '#[0-9]+' "$SPEC_DIR/spec.md" 2>/dev/null | head -1)
CLOSES_LINE=""
