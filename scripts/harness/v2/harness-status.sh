...
show_project_summary() {
  ...
  if [[ -f "$proj_dir/.harness/spec-dir" ]]; then
    ...
    if [[ -f "$spec_dir/tasks.md" ]]; then
      ...
      done_count=$(grep -c '^\- \[x\] T[0-9]' "$spec_dir/tasks.md" 2>/dev/null || echo 0)
      total_tasks=$((done_count + $(grep -c '^\- \[ \] T[0-9]' "$spec_dir/tasks.md" 2>/dev/null || echo 0)))
...
if [[ $# -eq 0 ]]; then
  ...
  if [[ -f "$PROJECT_DIR/.harness/spec-dir" ]]; then
    ...
    if [[ -f "$TASKS_MD" ]]; then
      ...
      echo ""
      echo "Tasks:"
      DONE=$(grep -c '^\- \[x\] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
      PENDING=$(grep -c '^\- \[ \] T[0-9]' "$TASKS_MD" 2>/dev/null || echo 0)
      TOTAL=$((DONE + PENDING))
